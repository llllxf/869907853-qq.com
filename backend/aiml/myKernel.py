...
class Kernel:
    ...
    def _check_all_english(self, des_str):
        #print(des_str,"checkenglish")
        for uchar in des_str:

            if (uchar >= u'\u0041' and uchar <= u'\u005a') or (uchar >= u'\u0061' and uchar <= u'\u007a'):
                pass
            else:
                print(uchar+"uchar",len(uchar))
                return False
        return True
    '''?#???*????0+??'''
            ...
            for key,tem in handler.categories.items():
                ...
                for k in key[0]:
                    if k=='#':
                        ...
                        check_english+=k
                #print(check_english,"check_english")
                if key and key[0] and key[1] and key[2] and em_ext == '.aiml' and (not self._check_all_english(check_english)):
                #if key and key[0X] and key[1] and key[2] and em_ext == '.aiml':
                ...
                elif key and key[0] and key[1] and key[2] and em_ext == '.aiml' and self._check_all_english(check_english):
