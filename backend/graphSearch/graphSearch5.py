...
class graphSearch(object):
    ...
    def getEntByfuzzySearch(self, description):
        ...
        uri = "http://localhost:8004/fuzzySearch?repertoryName=geo4&description=" + description
        r = requests.post(uri)
        ent_list = list(r.json())
    ...
    def getCompareKeyword(self, description,entity):
        ...
        uri = "http://localhost:8004/getCompareKeyword?repertoryName=geo4&description=" + description+"&entityname="+entity
        r = requests.post(uri)
        ent_list = list(r.json())
    ...
    def getEntityByType(self, etype):
        ...
        """获取子类"""
        uri = "http://localhost:8004/getEntityByType?repertoryName=geo4&entityName=" + etype
        r = requests.post(uri)
    ...
    def getEntityByRelLimitType(self, entity,property,keyword):
        ...
        uri = "http://localhost:8004/entitySearchByRelLimitType?repertoryName=geo4&entity="+entity+"&relation="+property+"&type="+keyword
        r = requests.post(uri)
        ent_list = list(r.json())
...
if __name__ == '__main__':
    ...
    for l in lines:
        ...
        arr.append({"subject": g.getPredicate(temp[0]), "predicate": 'http://purl.org/dc/elements/1.1/description', "object": temp[1].strip("\n")})

    g.addTripleToRepertory(arr)
    #print(arr)


    instanceArray = list(set(read_file(project_path + "/data/allentity.csv")))
    jieba.load_userdict(instanceArray)
    g = graphSearch()
    heliu = g.getEntityByType('民族')

    prolist = g.dealWithEnitity(heliu)
    ...
    for h in heliu:
        ...
        for p in pro:
            ...
            p_arr = list(jieba.cut(p[1]))
            if len(p_arr) == 1:
                continue
            for a in p_arr:
                if a in instanceArray:
                    ans.append(h + " " + p[0] + ":" + p[1] + "\n")
